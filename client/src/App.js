...
const toDate = (unix) => new Date(Number(unix) * 1000).toLocaleString("en-GB", {
...
const calcReward = (amountWei, startTime) => {
  ...
  return (BigInt(amountWei) * BigInt(ANNUAL_RATE) * elapsed) / (BigInt(YEAR_SECONDS) * BigInt(RATE_PRECISION));
};
...
const fetchChainData = async (address) => {
  ...
  return {
    stakes: raw.map((s, i) => ({
      index: i, amount: s.amount, startTime: s.startTime,
      lockDuration: s.lockDuration, withdrawn: s.withdrawn,
    })),
    contractBalance: bal,
  };
};
...
function NotificationBanner({ message, onClose, onLearnMore }) {
  ...
  return (
    <div style={{
      ...
      background: "#fff3cd", borderBottom: "3px solid #ffc107",
      padding: "14px 24px", display: "flex", alignItems: "center",
      justifyContent: "center", boxShadow: "0 2px 8px rgba(0,0,0,0.1)",
      gap: 16
    }}>
      <p style={{ margin: 0, fontSize: 14, color: "#856404", fontWeight: 500, flex: 1, textAlign: "center" }}>
        ⚠️ {message}
      </p>
      ...
      <button onClick={onClose} style={{
        ...
        color: "#856404", padding: "0 8px", lineHeight: 1, transition: "transform 0.2s"
...
function AuthPage({ onLogin }) {
  ...
  return (
    <div style={{ minHeight: "100vh", background: "linear-gradient(135deg, #667eea 0%, #764ba2 100%)", display: "flex", alignItems: "center", justifyContent: "center", padding: 20 }}>
      <div style={{ background: "#fff", borderRadius: 20, padding: "50px 40px", maxWidth: 440, width: "100%", boxShadow: "0 20px 60px rgba(0,0,0,0.3)" }}>
        <div style={{ width: 80, height: 80, margin: "0 auto 24px", background: "linear-gradient(135deg, #667eea 0%, #764ba2 100%)", borderRadius: "50%", display: "flex", alignItems: "center", justifyContent: "center" }}>
        ...
        {error && (
          <div style={{ background: "#fee", border: "1px solid #fcc", borderRadius: 8, padding: 12, marginBottom: 20, fontSize: 14, color: "#c00" }}>
            {error}
          </div>
        )}
        ...
        <form onSubmit={handleSubmit}>
          <div style={{ marginBottom: 20 }}>
            <label style={{ display: "block", fontSize: 14, fontWeight: 600, color: "#2d3748", marginBottom: 8 }}>Username</label>
            <input type="text" value={username} onChange={(e) => setUsername(e.target.value)} placeholder="Enter username"
              style={{ width: "100%", padding: 14, fontSize: 15, border: "2px solid #e2e8f0", borderRadius: 10, fontFamily: "inherit", outline: "none", boxSizing: "border-box", transition: "all 0.2s" }}
              onFocus={(e) => { e.target.style.borderColor = "#667eea"; e.target.style.transform = "scale(1.01)"; }}
              onBlur={(e) => { e.target.style.borderColor = "#e2e8f0"; e.target.style.transform = "scale(1)"; }} />
          </div>
          ...
          {!isLogin && (
            <div style={{ marginBottom: 20 }}>
              <label style={{ display: "block", fontSize: 14, fontWeight: 600, color: "#2d3748", marginBottom: 8 }}>Confirm Password</label>
          ...
          <button type="submit" 
            ...
            background: pressedBtn ? "linear-gradient(135deg, #5568d3 0%, #6a3f8f 100%)" : "linear-gradient(135deg, #667eea 0%, #764ba2 100%)", 
            border: "none", borderRadius: 10, cursor: "pointer", fontFamily: "inherit", 
            transform: pressedBtn ? "scale(0.97)" : hoveredBtn ? "scale(1.03)" : "scale(1)",
            boxShadow: pressedBtn ? "0 2px 8px rgba(102,126,234,0.3)" : hoveredBtn ? "0 8px 24px rgba(102,126,234,0.4)" : "0 4px 16px rgba(102,126,234,0.3)",
        ...
        <div style={{ marginTop: 24, textAlign: "center", fontSize: 14, color: "#718096" }}>
          ...
          <button onClick={() => { setIsLogin(!isLogin); setError(""); }}
            style={{ background: "transparent", border: "none", color: "#667eea", fontWeight: 600, cursor: "pointer", textDecoration: "underline", fontSize: 14 }}>
...
function ConnectWalletPage({ onConnect, username }) {
  ...
  return (
    <div style={{ minHeight: "100vh", background: "linear-gradient(135deg, #667eea 0%, #764ba2 100%)", display: "flex", alignItems: "center", justifyContent: "center", padding: 20 }}>
      <div style={{ background: "#fff", borderRadius: 20, padding: "60px 50px", maxWidth: 480, width: "100%", boxShadow: "0 20px 60px rgba(0,0,0,0.3)", textAlign: "center" }}>
        <div style={{ fontSize: 64, marginBottom: 20 }}>🦊</div>
        <h1 style={{ fontSize: 32, fontWeight: 700, color: "#1a202c", marginBottom: 12 }}>Connect Wallet</h1>
        <p style={{ fontSize: 16, color: "#4a5568", marginBottom: 8 }}>Welcome, <strong>{username}</strong>!</p>
        <p style={{ fontSize: 16, color: "#718096", marginBottom: 40 }}>Connect MetaMask to start staking</p>
        <button 
          onMouseDown={() => setPressed(true)}
          onMouseUp={() => setPressed(false)}
          onMouseEnter={() => setHovered(true)}
          onMouseLeave={() => { setHovered(false); setPressed(false); }}
          onClick={onConnect} style={{
          width: "100%", padding: "18px 32px", fontSize: 16, fontWeight: 700, color: "#fff",
          background: pressed ? "linear-gradient(135deg, #5568d3 0%, #6a3f8f 100%)" : "linear-gradient(135deg, #667eea 0%, #764ba2 100%)", 
          border: "none", borderRadius: 12, cursor: "pointer",
          transform: pressed ? "scale(0.97)" : hovered ? "scale(1.03)" : "scale(1)",
          boxShadow: pressed ? "0 2px 8px rgba(102,126,234,0.3)" : hovered ? "0 8px 24px rgba(102,126,234,0.4)" : "0 4px 16px rgba(102,126,234,0.3)",
...
function LearnPage({ onBack }) {
  ...
  const topics = [
    ...
    { icon: "⛽", title: "Gas Fees", text: "Small fees paid to validators for processing transactions. On local Hardhat network, gas is free for testing." },
  ...
  const faqs = [
    ...
    { q: "Can I withdraw just rewards?", a: "No, withdrawal includes both principal and rewards together." }
  ];

  return (
    <div style={{ background: "#f7f9fc", minHeight: "100vh", paddingBottom: 80 }}>
      <div style={{ background: "linear-gradient(135deg, #667eea 0%, #764ba2 100%)", padding: "clamp(30px, 5vw, 50px) 24px", borderBottom: "4px solid #5a67d8" }}>
        <div style={{ maxWidth: 1100, margin: "0 auto" }}>
          <button onClick={onBack} style={{
            background: "rgba(255,255,255,0.2)", border: "2px solid rgba(255,255,255,0.4)", color: "#fff",
            padding: "10px 20px", fontSize: 14, fontWeight: 600, cursor: "pointer", borderRadius: 8,
      ...
      <div style={{ maxWidth: 1100, margin: "0 auto", padding: "clamp(30px, 5vw, 60px) 24px" }}>
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(280px, 1fr))", gap: 24 }}>
          {topics.map((t, i) => (
            <div key={i} onMouseEnter={() => setHoveredCard(i)} onMouseLeave={() => setHoveredCard(null)}
              style={{
                background: "#fff", border: "2px solid #e2e8f0", borderRadius: 16, padding: 28,
                transition: "all 0.3s ease", cursor: "pointer",
                transform: hoveredCard === i ? "translateY(-8px)" : "translateY(0)",
                boxShadow: hoveredCard === i ? "0 12px 32px rgba(102,126,234,0.15)" : "0 2px 8px rgba(0,0,0,0.05)"
              }}>
        ...
        <div style={{ marginTop: 60 }}>
          ...
          {faqs.map((faq, i) => {
            ...
            return (
              <div key={i} style={{ background: "#fff", border: "2px solid #e2e8f0", borderRadius: 12, overflow: "hidden", marginBottom: 16 }}>
                <button onClick={() => setExpandedFAQ(isExpanded ? null : i)} style={{
                  ...
                  alignItems: "center", fontFamily: "inherit", fontSize: 16, fontWeight: 600, color: "#2d3748"
                }}>
                  <span>{faq.q}</span>
                  <span style={{ fontSize: 20, transition: "transform 0.2s", transform: isExpanded ? "rotate(180deg)" : "rotate(0)" }}>▼</span>
                </button>
                <div style={{ maxHeight: isExpanded ? 200 : 0, overflow: "hidden", transition: "max-height 0.3s ease" }}>
                  <p style={{ padding: "0 20px 20px", fontSize: 15, color: "#4a5568", lineHeight: 1.7 }}>{faq.a}</p>
                </div>
              </div>
            );
          })}
  ...
  return (
    <div style={{ background: "#f7f9fc", minHeight: "100vh", paddingBottom: 80 }}>
      ...
      <div style={{ maxWidth: 900, margin: "0 auto", padding: "clamp(30px, 5vw, 60px) 24px" }}>
        <div style={{ background: "#fff", border: "2px solid #e2e8f0", borderRadius: 16, padding: "clamp(24px, 4vw, 48px)", boxShadow: "0 4px 16px rgba(0,0,0,0.06)" }}>
          <div style={{ marginBottom: 32 }}>
            ...
            <input type="number" value={amount} onChange={(e) => setAmount(e.target.value)} min="0" step="0.01"
              style={{ width: "100%", padding: 16, fontSize: 18, border: "2px solid #e2e8f0", borderRadius: 10, boxSizing: "border-box", outline: "none", transition: "all 0.2s" }}
              onFocus={(e) => { e.target.style.borderColor = "#667eea"; e.target.style.transform = "scale(1.01)"; }}
              onBlur={(e) => { e.target.style.borderColor = "#e2e8f0"; e.target.style.transform = "scale(1)"; }} />
          </div>
          ...
          <div style={{ marginBottom: 40 }}>
            ...
            <input type="range" min="1" max="36" value={months} onChange={(e) => setMonths(e.target.value)}
          ...
          <div style={{ background: "linear-gradient(135deg, #f7fafc 0%, #edf2f7 100%)", padding: "clamp(20px, 3vw, 32px)", borderRadius: 12 }}>
            <div style={{ marginBottom: 20 }}>
            ...
            <div style={{ marginBottom: 20 }}>
              <div style={{ fontSize: 13, color: "#718096", marginBottom: 6 }}>Estimated Rewards ({ANNUAL_RATE}% APR)</div>
            ...
            <div style={{ borderTop: "2px solid #cbd5e0", paddingTop: 20, marginTop: 20 }}>
              <div style={{ fontSize: 13, color: "#718096", marginBottom: 6 }}>Total After {months} Months</div>
              <div style={{ fontSize: "clamp(24px, 4vw, 36px)", fontWeight: 700, color: "#2d3748", marginBottom: 8 }}>
                {total.toFixed(6)} ETH
              </div>
              {ethPrice > 0 && (
                <div style={{ fontSize: 16, color: "#4a5568" }}>≈ ${(total * ethPrice).toFixed(2)} USD</div>
              )}
            </div>
          </div>
        </div>
...
function MainApp({ account, username, onLogout }) {
  ...
  const [notification, setNotification] = useState("Don't invest unless you're prepared to lose all the money you invest. This is a high-risk platform and you should not expect to be protected if something goes wrong.");
  ...
  if (priceHistory.length > 0) {
    ...
    pathData = priceHistory.map((p, i) => {
      const x = padding + (i / (priceHistory.length - 1)) * (chartWidth - padding * 2);
      const y = padding + (chartHeight - padding * 2) - ((p[1] - minPrice) / range) * (chartHeight - padding * 2);
      return `${i === 0 ? 'M' : 'L'} ${x} ${y}`;
    }).join(' ');
  }
  ...
  return (
    <div style={{ background: "#f7f9fc", minHeight: "100vh" }}>
      <nav style={{
        background: "#fff", height: 70, display: "flex", alignItems: "center", justifyContent: "space-between",
        padding: "0 clamp(16px, 3vw, 40px)", position: "sticky", top: 0, zIndex: 100,
        borderBottom: "2px solid #e2e8f0", boxShadow: "0 2px 8px rgba(0,0,0,0.05)"
      }}>
        <div onClick={() => setPage("home")} style={{ display: "flex", alignItems: "center", gap: 12, cursor: "pointer" }}>
          ...
          <span style={{ fontSize: "clamp(14px, 2vw, 17px)", fontWeight: 700, color: "#2d3748" }}>ETH Staking</span>
        </div>
        ...
        <div style={{ display: "flex", gap: "clamp(8px, 1.5vw, 16px)" }}>
          {[
            ...
            <button key={label} onClick={() => setPage(action)}
              onMouseEnter={() => setHoveredBtn(action)}
              onMouseLeave={() => setHoveredBtn(null)}
              style={{
                padding: "8px 16px", background: hoveredBtn === action ? "#f7fafc" : "transparent",
                border: "none", cursor: "pointer", fontSize: "clamp(12px, 1.5vw, 14px)",
                fontWeight: 600, color: hoveredBtn === action ? "#667eea" : "#4a5568",
        ...
        <div style={{ display: "flex", gap: 12, alignItems: "center" }}>
          <span style={{ fontSize: "clamp(12px, 1.5vw, 14px)", fontWeight: 600, color: "#2d3748" }}>{username}</span>
          <button onClick={onLogout} 
            onMouseEnter={() => setHoveredBtn("logout")}
            onMouseLeave={() => setHoveredBtn(null)}
            style={{
            padding: "8px 16px", background: hoveredBtn === "logout" ? "#e53e3e" : "transparent",
            color: hoveredBtn === "logout" ? "#fff" : "#e53e3e", border: "2px solid #e53e3e",
            borderRadius: 8, cursor: "pointer", fontSize: "clamp(11px, 1.5vw, 13px)",
      ...
      <div style={{ marginTop: notification ? 60 : 0 }}>
        <div style={{
          ...
          padding: "clamp(40px, 6vw, 80px) 24px", borderBottom: "4px solid #5a67d8"
        }}>
          <div style={{ maxWidth: 1100, margin: "0 auto" }}>
            <h1 style={{ fontSize: "clamp(32px, 6vw, 56px)", fontWeight: 700, color: "#fff", marginBottom: 20 }}>
              Ethereum Staking
            </h1>
            <p style={{ fontSize: "clamp(16px, 2.5vw, 22px)", color: "rgba(255,255,255,0.9)", marginBottom: 40 }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(200px, 1fr))", gap: 20 }}>
              <div style={{
                ...
                padding: 24, borderRadius: 12, border: "2px solid rgba(255,255,255,0.2)"
              }}>
                <div style={{ fontSize: 13, color: "rgba(255,255,255,0.8)", marginBottom: 8 }}>Total Staked</div>
              ...
              <div style={{
                ...
                <div style={{ fontSize: 13, color: "rgba(255,255,255,0.8)", marginBottom: 8 }}>Rewards Earned</div>
                <div style={{ fontSize: "clamp(20px, 3vw, 28px)", fontWeight: 700, color: "#48bb78" }}>
              ...
              <div style={{
                ...
                <div style={{ fontSize: 13, color: "rgba(255,255,255,0.8)", marginBottom: 8 }}>Annual Rate</div>
                <div style={{ fontSize: "clamp(20px, 3vw, 28px)", fontWeight: 700, color: "#fff" }}>
                  {ANNUAL_RATE}% APR
                </div>
              </div>
        ...
        <div style={{ background: "#fff", borderBottom: "1px solid #e2e8f0" }}>
          <div style={{ maxWidth: 1200, margin: "0 auto", padding: "clamp(40px, 6vw, 80px) 24px" }}>
            ...
            <p style={{ fontSize: "clamp(14px, 2vw, 16px)", color: "#718096", marginBottom: 48 }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(300px, 1fr))", gap: 32 }}>
              <div style={{ textAlign: "center" }}>
                ...
                <p style={{ fontSize: 15, color: "#4a5568", lineHeight: 1.7 }}>
                  Stake any amount of ETH with just a few clicks. No minimum requirements, instant rewards.
                </p>
              </div>
              ...
              <div style={{ textAlign: "center" }}>
                ...
                <p style={{ fontSize: 15, color: "#4a5568", lineHeight: 1.7 }}>
                  Direct blockchain interaction. Transparent, secure, and fully decentralized staking.
                </p>
              </div>
        ...
        <div style={{ maxWidth: 1400, margin: "0 auto", padding: "clamp(30px, 5vw, 60px) 24px" }}>
          <div style={{
            display: "grid",
            ...
            <div>
              ...
              <div style={{
                ...
                padding: "clamp(24px, 4vw, 40px)", marginBottom: 32, boxShadow: "0 4px 16px rgba(0,0,0,0.06)"
              }}>
                ...
                <input type="number" value={stakeAmount} onChange={(e) => setStakeAmount(e.target.value)}
                  ...
                  style={{
                    width: "100%", padding: 16, fontSize: 18, border: "2px solid #e2e8f0",
                    borderRadius: 10, marginBottom: 20, boxSizing: "border-box", outline: "none",
                    transition: "all 0.2s"
                  }}
                  onFocus={(e) => {
                ...
                <button 
                  ...
                  style={{
                    width: "100%", padding: 16, fontSize: 16, fontWeight: 700, color: "#fff",
                    background: !account ? "#cbd5e0" : pressedBtn === "stake" ? "linear-gradient(135deg, #5568d3 0%, #6a3f8f 100%)" : "linear-gradient(135deg, #667eea 0%, #764ba2 100%)",
                    border: "none", borderRadius: 10, cursor: account ? "pointer" : "not-allowed",
                    transform: pressedBtn === "stake" ? "scale(0.97)" : hoveredBtn === "stake" ? "scale(1.02)" : "scale(1)",
                    boxShadow: pressedBtn === "stake" ? "0 2px 8px rgba(102,126,234,0.3)" : hoveredBtn === "stake" ? "0 6px 20px rgba(102,126,234,0.4)" : "0 2px 8px rgba(102,126,234,0.2)",
                    transition: "all 0.15s ease"
                  }}>
                ...
                {status && (
                  <div style={{
                    marginTop: 16, padding: 12, borderRadius: 8, fontSize: 14,
                    background: status.includes("✅") ? "#d4edda" : status.includes("❌") ? "#f8d7da" : "#edf2f7",
                    color: status.includes("✅") ? "#155724" : status.includes("❌") ? "#721c24" : "#2d3748"
                  }}>
                    {status}
                  </div>
                )}
              ...
              <div style={{
                background: "#fff", border: "2px solid #e2e8f0", borderRadius: 16,
                padding: "clamp(24px, 4vw, 40px)", boxShadow: "0 4px 16px rgba(0,0,0,0.06)"
              }}>
                <h2 style={{ fontSize: "clamp(24px, 3vw, 32px)", fontWeight: 700, color: "#2d3748", marginBottom: 24 }}>
                  My Stakes
                </h2>
                ...
                {stakes.length === 0 ? (
                  <div style={{ padding: 48, textAlign: "center", background: "#f7fafc", borderRadius: 12, color: "#718096" }}>
                    ...
                    <table style={{ width: "100%", borderCollapse: "collapse", minWidth: 800 }}>
                      <thead>
                        <tr style={{ borderBottom: "2px solid #e2e8f0", background: "#f7fafc" }}>
                          <th style={{ padding: 16, textAlign: "left", fontWeight: 600, color: "#4a5568" }}>#</th>
                          <th style={{ padding: 16, textAlign: "left", fontWeight: 600, color: "#4a5568" }}>Amount</th>
                          <th style={{ padding: 16, textAlign: "left", fontWeight: 600, color: "#4a5568" }}>Started</th>
                          <th style={{ padding: 16, textAlign: "left", fontWeight: 600, color: "#4a5568" }}>Rewards</th>
                          <th style={{ padding: 16, textAlign: "left", fontWeight: 600, color: "#4a5568" }}>Status</th>
                          <th style={{ padding: 16, textAlign: "left", fontWeight: 600, color: "#4a5568" }}>Action</th>
                        </tr>
                      </thead>
                      <tbody>
                        {stakes.map((s) => {
                          ...
                          return (
                            <tr key={s.index} style={{ borderBottom: "1px solid #e2e8f0" }}>
                              ...
                              <td style={{ padding: 16, fontWeight: 700 }}>{toEth4(s.amount)} ETH</td>
                              ...
                              <td style={{ padding: 16 }}>
                                <button 
                                  ...
                                  onMouseLeave={() => { setHoveredBtn(null); setPressedBtn(null); }}
                                  style={{
                                    background: canW ? (pressedBtn === `w${s.index}` ? "#5568d3" : "#667eea") : "#e2e8f0",
                                    color: canW ? "#fff" : "#a0aec0", border: "none", padding: "10px 20px",
                                    fontSize: 13, fontWeight: 700, cursor: canW ? "pointer" : "not-allowed",
                                    borderRadius: 8, transition: "all 0.15s",
                                    transform: pressedBtn === `w${s.index}` ? "scale(0.95)" : hoveredBtn === `w${s.index}` && canW ? "scale(1.05)" : "scale(1)"
                                  }}>
            ...
            {/* Sidebar */}
            <div>
              <div style={{ position: window.innerWidth > 1024 ? "sticky" : "static", top: notification ? 148 : 88 }}>
                <div style={{
                  background: "#fff3cd", border: "2px solid #ffc107", borderRadius: 12,
                ...
                <div style={{
                  ...
                  {priceHistory.length > 0 && (
                    <svg width="100%" height="140" viewBox={`0 0 ${chartWidth} ${chartHeight}`}>
                      ...
                      <path d={`${pathData} L ${chartWidth-padding} ${chartHeight-padding} L ${padding} ${chartHeight-padding} Z`} fill="url(#grad)" />
                ...
                <div style={{ background: "#fff", border: "2px solid #e2e8f0", borderRadius: 12, padding: 20 }}>
                  ...
                  <div style={{ fontSize: 13, lineHeight: 1.8 }}>
                    ...
                    <div><strong>APR:</strong> {ANNUAL_RATE}%</div>
                  </div>
                </div>
              </div>
            </div>
        ...
        <footer style={{
          background: "#2d3748", borderTop: "4px solid #667eea", padding: 24,
          textAlign: "center", color: "#a0aec0", fontSize: 14
        }}>
...
export default function App() {
  ...
  if (stage === "auth") return <AuthPage onLogin={(u) => { setUsername(u); setStage("connect"); }} />;
  if (stage === "connect") return <ConnectWalletPage username={username} onConnect={connectWallet} />;
  return <MainApp account={account} username={username} onLogout={() => { setAccount(null); setUsername(""); setStage("auth"); }} />;
}
